...
uint64_t getSize(uint64_t empty1, uint64_t empty2, uint64_t empty3, uint64_t empty4, uint64_t empty5){
    return getFontSize();
}
...
static uint64_t (*syscalls[])(uint64_t rdi, uint64_t rsi, uint64_t rdx, uint64_t r10, uint64_t r8) =
        {read, readCharFromBuffer, write,customWrite, setCursorPosition,getCurrentTime, getScrWidth, getScrHeight, drawFilledRect, dottedLine, videoRefresh, clearScreen, playBeep, drawVideoChar, getPtrToPixel,zoomIn,zoomOut,getSize,drawCustomNumber,CurrentKeyPressed};
...
uint64_t CurrentKeyPressed(uint64_t rdi, uint64_t empty2, uint64_t empty3, uint64_t empty4, uint64_t empty5){
getAllKeys((char*)rdi);
}
